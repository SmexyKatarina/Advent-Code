...
function part1(input: string): number {
    ...
    const papers = grid
        ...
        .filter(cell => cell.cellValue === "@") // Filter the cells for only paper
        .filter(cell =>
...
function part2(input: string): number {
    ...
    while (true) {
        ...
        const accessablePapers = grid
            ...
            .flat() // Flatten the array
            .filter(cell => {
                // If not a paper cell, skip it.
                if (cell.cellValue !== "@") return false;
                // Then check this paper cell and its adjacent cells
                return grid.getAdjacentCells(cell) // And that the adjacent cells to this one
                    .filter(y => y.cellValue === "@") // Where the ones that are paper
                    .length < 4 // Are totaled to less than 4
            })
